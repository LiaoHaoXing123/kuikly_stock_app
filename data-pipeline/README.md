...
GitHub Actions（每天 07:30 UTC = 北京 15:30，周一~五）
        │  build_stock_db.py：AKShare 抓数 -> SQLite stock.db（六张表）
        │  只保留最近 30 天（K线/分时/指标自动剪枝），并标注每张表的数据来源
        ▼
...
- `stock.db`：App 读取的 SQLite 库。六张表见 `build_stock_db.py` 的 `DDL`。
...
可用环境变量调整：`KLINE_CODES` / `MINUTE_CODES` / `ORDERBOOK_CODES`
（关注列表，逗号分隔）、`KLINE_DAYS_BACK`（默认 60，用于算指标）、`PRUNE_DAYS`（默认 30）。
...
`PRUNE_DAYS=30`：`stock_daily_kline` / `stock_minute` / `stock_indicator` 只保留最近 30 个自然日；
