...
class RiskCenterPage : BasePager() {
    ...
    internal fun confirmAdd() {
        ...
        val kept = WatchStore.find(code)
        WatchStore.updateHolding(WatchHolding(code, name, shares, cost, kept?.startDate.orEmpty(), kept?.endDate.orEmpty()))
        showAddDialog = false
        addMessage = ""
    ...
    internal fun confirmEdit() {
        ...
        val kept = WatchStore.find(input.code)
        val period = if (input.shares > 0) kept?.startDate.orEmpty() to kept?.endDate.orEmpty() else "" to ""
        WatchStore.updateHolding(WatchHolding(input.code, editName, input.shares, input.cost, period.first, period.second))
