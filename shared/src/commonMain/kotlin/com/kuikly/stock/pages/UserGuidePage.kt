                ...
                Scroller {
                    ...
                    guideSection(ctx, "g02", "二", "五个主要入口", "各 Tab 能做什么") {
                        guideTable(
                            ...
                            listOf(
                                listOf("首页", "盘面概况、今日关注、研究工作台、盈亏日历与刷新", "每次打开先确认数据日期"),
                                listOf("行情", "搜索股票，按涨幅/跌幅/成交量排序，进个股详情", "找股票、看走势"),
                                listOf("研究", "与 AI 对话、追问、管理历史会话", "提出具体研究问题"),
                                listOf("自选仓", "管理关注股票、持仓信息和提醒", "持续跟踪少量标的"),
                    ...
                    guideSection(ctx, "g04", "四", "个股详情：从上到下怎么看", "区域与使用要点") {
                        ...
                        guideTable(
                            listOf("区域", "主要内容", "使用要点"),
                            listOf(
                                listOf("标题栏", "名称、代码、星标、AI分析", "星标加入或移除自选仓"),
                    ...
                    guideSection(ctx, "g10", "十", "分析历史与自选提醒", "历史、持仓、提醒、日历") {
                        ...
                        guidePara("自选仓：在个股详情标题栏点击星标加入，再次点击可移除。进入“自选仓”管理关注股票。")
                        guidePara("持仓：在自选仓中录入股数与成本，供市值、盈亏、组合风险和盈亏日历使用。这是本地记录，不会连接券商账户或执行交易。")
                        guidePara("提醒：可设置价格涨破、跌破或涨跌幅阈值；AI价位提供提醒入口时，先核对方向和阈值再保存。")
                        guidePara("盈亏日历：从首页、自选总览或“我的”进入。每次行情快照更新时，自动记下当天持仓市值和当日盈亏；无持仓的日子不落记录，也不会用 K 线回填接入前的历史。同一天再次更新会覆盖当天快照。")
                        guideBullet("点某一天可看各股贡献；格子旁标跑赢或跑输沪深300。")
                    ...
                    guideSection(ctx, "g11", "十一", "组合风险", "市值、集中度与回撤") {
                        guidePara("从首页研究工作台进入“组合风险”。先在自选仓中录入持仓，否则可能只显示空状态。")
                        guidePara("重点查看：")
                        guideBullet("组合市值、盈亏。")
                    ...
                    guideSection(ctx, "g14", "十四", "一次完整的研究操作示例", "十分钟走完一个研究循环") {
                        ...
                        guideStep("9", "将股票加入自选仓，按需要记录持仓或设置提醒。")
