...
internal fun ViewContainer<*, *>.detailAction(label: String, action: () -> Unit) {
    View {
        attr { padding(8f, 10f, 8f, 10f); marginTop(4f); marginRight(6f); borderRadius(8f); backgroundColor(AppColor.INFO_BG) }
        event { click { action() } }
        Text { attr { text(label); fontSize(12f); color(AppColor.PRIMARY_SOFT) } }
    }
}

internal fun ViewContainer<*, *>.analysisHistoryPanel(state: DetailAnalysisState, onChange: () -> Unit) {
    View {
        attr { padding(12f); margin(6f, 12f, 6f, 12f); backgroundColor(AppColor.SURFACE); borderRadius(10f) }
        Text { attr { text("分析记录 · ${state.records.size} 条"); fontSize(14f); fontWeightBold(); color(AppColor.TEXT_DEEP) } }
        Text {
            attr {
                ...
                fontSize(11f); lineHeight(17f); color(AppColor.TEXT_SUB_DEEP); marginTop(6f)
            }
        }
        Text { attr { text(state.notice); fontSize(11f); color(AppColor.WARNING_TEXT); marginTop(4f) } }
        ...
        vif({ state.expanded }) {
            Scroller {
                ...
                vfor({ ObservableList(state.records.toMutableList()) }) { record ->
                    View {
                        attr { padding(8f); marginBottom(5f); backgroundColor(if (state.selectedId == record.id) AppColor.PRIMARY_BG_LIGHT else AppColor.SURFACE_ALT); borderRadius(8f) }
                        Text { attr { text("${exportTimestampString(record.result.generatedAt)} · ${record.result.source}"); fontSize(11f); color(AppColor.TEXT_DEEP) } }
                        View {
...
internal fun ViewContainer<*, *>.chartEvidencePanel(
    ...
    View {
        attr { marginTop(8f); padding(10f); backgroundColor(AppColor.PRIMARY_BG_LIGHT); borderRadius(10f) }
        Text { attr { text("行情依据 · 与K线联动"); fontSize(13f); fontWeightBold(); color(AppColor.TEXT_DEEP) } }
        Text { attr { text(candleEvidence(bars(), selected())); fontSize(11f); lineHeight(17f); color(AppColor.TEXT_SUB_DEEP); marginTop(6f) } }
        Text {
            attr {
                ...
                fontSize(11f); lineHeight(17f); color(AppColor.VIOLET); marginTop(6f)
            }
        }
        detailAction("携带选中行情问 AI") { ask() }
        Text { attr { text("以下为行情计算的候选信号，点击定位；并非模型已验证的结论"); fontSize(10f); color(AppColor.TEXT_SUB_DEEP); marginTop(8f) } }
...
internal fun ViewContainer<*, *>.aiEvidencePanel(analysis: () -> AIAnalysisData?, focusDate: (String) -> Unit) {
    View {
        attr { padding(10f); marginTop(8f); backgroundColor(AppColor.PRIMARY_BG_LIGHT); borderRadius(10f) }
        Text { attr { text("AI 引用的行情依据"); fontSize(13f); fontWeightBold(); color(AppColor.TEXT_DEEP) } }
        vif({ aiDatedEvidence(analysis()).isEmpty() }) {
            Text { attr { text("本次分析未提供可定位日期。可在图表选择行情后追问。"); fontSize(11f); color(AppColor.TEXT_SUB_DEEP); marginTop(6f) } }
        }
        vfor({ ObservableList(aiDatedEvidence(analysis()).toMutableList()) }) { evidence ->
            View {
                attr { flexDirectionColumn(); marginTop(8f) }
                Text { attr { text(evidence.reason); fontSize(12f); lineHeight(18f); color(AppColor.TEXT_SUB_DEEP) } }
